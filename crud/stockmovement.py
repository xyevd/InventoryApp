...
async def create_movement(db: AsyncSession, data: StockMovementCreate) -> StockMovement:
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(status_code=400, detail=str(getattr(e.orig, "args", ["Ошибка целостности"])[0]))
    await db.refresh(movement)
    return movement

async def list_movements(db: AsyncSession) -> List[StockMovement]:
    res = await db.execute(select(StockMovement))
